...
MainWindowPhotoShop::MainWindowPhotoShop(QWidget *parent) : QMainWindow(parent),ui(new Ui::MainWindowPhotoShop)
{
  ...
  // Etape 14 (TO DO)
  PhotoShop::getInstance().Load("sauvegarde.dat"); //On récupère les données du fichier 
...
void MainWindowPhotoShop::closeEvent(QCloseEvent *event)
{
  ...
  {
    string enregistrement=dialogueDemandeTexte("Quitter","Souhaitez-vous enregistrer vos images (O/N)?");

    if (enregistrement == "o" || enregistrement == "O")
    {
